...
void game(){
    ...
    int listOfQuestions[31]; //we will given numbers to list of 30 questions
    for(int i = 0; i<31; i++){
        listOfQuestions[i] = i+1;
    }
    //making a random list form a simple 1-30 list
    for(int j = 31; j>0; j-- ){
        z = rand()%j;
        temp =  listOfQuestions[j-1];
        listOfQuestions[j-1] = listOfQuestions[z];
        listOfQuestions[z] = temp;
    }
    ...
    q.readQuestionFromFile(listOfQuestions[qNumber]);
    q.displayQuestion();
    q.correctGiven = false;
    q.lifeLineUsed = false;
    q.answer();
